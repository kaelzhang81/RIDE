...
#         self.keyword_cache = ExpiringCache()
    ...
    def get_keywords_from(self, datafile, ctx):
        ...
        datafile_keywords = self._get_datafile_keywords(datafile)
        resource_keywords = self._get_imported_resource_keywords(datafile, ctx)
        library_keywords = self._get_imported_library_keywords(datafile, ctx)
        words = sorted(set(datafile_keywords + resource_keywords + library_keywords))
        return words
    ...
    def _collect_kws_from_imports(self, datafile, instance_type, getter, ctx):
        kws = []
        collect = self._collect_import_of_type(datafile, instance_type)
        for imp in collect:
    ...
    def get_keywords_cached(self, datafile, context_factory):
        ...
        if not values:
            print('start cache:{0}'.format(datetime.datetime.now()))
